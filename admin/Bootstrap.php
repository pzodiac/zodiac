...
class Bootstrap
{
    ...
    public function run()
    {
        ...
        $di->set('db', $db);
        // Config
        require_once CORE_PATH . '/modules/Cms/Config.php';
        $config2 = \Cms\Config::get();
        ...
        $loader->registerDirs([CORE_PATH . "/plugins/"]);
        ...
        $registry->cms = $cmsModel->getConfig(); // Gửi đến Registry
    ...
    private function initEventManager($di)
    {
        ...
        $eventsManager->attach("dispatch:beforeDispatchLoop", function ($event, $dispatcher) use ($di) {
            new \Zodiac\Plugin\CheckPoint($di->get('request'));
            new \Zodiac\Plugin\Localization($dispatcher);
            new \Zodiac\Plugin\AdminLocalization($di->get('config'));
            new \Zodiac\Plugin\Acl($di->get('acl'), $dispatcher, $di->get('view'));
            new \Zodiac\Plugin\MobileDetect($di->get('session'), $di->get('view'), $di->get('request'));
        });
    ...
    private function initView($di)
    {
        ...
        $volt->setOptions(['compiledPath' => APPLICATION_PATH . '/../data/cache/volt/backend/']);
        $volt->initCompiler();
